...
## Steps to Complete Day 2

1. **Step 1:** You only need the primary goal to complete Day 2; the Advanced Challenge is for going the extra mile.
2. **Step 2:** Successfully connect to the coffee shop barista in your browser and place a coffee order.
3. **Step 3:** Record a short video of your session placing a coffee order with the agent and show the JSON file summarizing the order.
...
✅ **Barista Persona:** Friendly Murf Coffee barista
✅ **Order State Management:** Full state tracking with all required fields
✅ **Function Tools:** 6 tools for order management:
- `update_drink_type()` - Set the beverage type
- `update_size()` - Set small/medium/large
- `update_milk()` - Set milk preference
- `add_extra()` - Add extras like whipped cream
- `update_customer_name()` - Set customer name
- `save_order()` - Save completed order to JSON

✅ **JSON Order Saving:** Orders saved to `backend/orders/` directory
✅ **Order Validation:** Checks all required fields before saving
✅ **Conversational Flow:** Natural question-asking with clarifications
...
3. Click "Start call"
4. Place a coffee order naturally (e.g., "I'd like a large latte with oat milk")
5. Check `backend/orders/` folder for the saved JSON file
